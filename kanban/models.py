...
class Milestone(models.Model):
    ...
    Represents key project events, deliverables, or deadlines
    """
    ...
    board = models.ForeignKey(Board, on_delete=models.CASCADE, related_name='milestones')
    title = models.CharField(max_length=200, help_text="Milestone name")
    description = models.TextField(blank=True, null=True, help_text="Detailed description of the milestone")
    ...
    completed_date = models.DateField(blank=True, null=True, help_text="Actual completion date")
    ...
    related_tasks = models.ManyToManyField(
        ...
        blank=True,
        help_text="Tasks that must be completed for this milestone"
    )
    
    # Metadata
    created_by = models.ForeignKey(User, on_delete=models.CASCADE, related_name='milestone_creations')
    created_at = models.DateTimeField(auto_now_add=True)
    updated_at = models.DateTimeField(auto_now=True)
    
    # Visual customization
    color = ColorField(default='#FFD700', help_text="Color for milestone marker on Gantt chart")
    ...
    def status(self):
        ...
        if self.is_completed:
            ...
            return 'Overdue'
        elif (self.target_date - timezone.now().date()).days <= 7:
            return 'Due Soon'
        else:
            return 'On Track'
    ...
    def mark_complete(self, user=None):
        ...
        if user:
            ...
            # Log completion (you can create a MilestoneActivity model if needed)
